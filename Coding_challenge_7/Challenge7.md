...
``` r
lmodel1 <- lm(Emergence ~Treatment*DaysAfterPlanting, data = plant)
summary(lmodel1)
```
...
coefficient for Treatment 2.
    ...
    Since all of the interaction terms are not significant from the interaction model, we do not need to include the term term in the model.

``` r
lmodel2 <- lm(Emergence ~Treatment + DaysAfterPlanting, data = plant)
summary(lmodel2)
```
...
```
    ...
    The interacept shows that there is an increases of 182.17 at treatment 1, which is the refefence
    The coefficient for treatment 2 shows that there is a decreases of 134.53 in emergence from treatment 1 and this decrese is highly significant P <2e-16
...
``` r
lsmeans <- emmeans(lmodel2, ~Treatment)
...
```
    ...
    According to the treatment comparisons,treatment 3,5,6 and have signidcantly higher means than treatment 7 and 2, while not different to treatments 1,4,8 & 9. Also, treatment 2 is significantly different from 7.
...
plot_cldbars_onefactor(lmodel1, "Treatment")
```

    ## NOTE: Results may be misleading due to involvement in interactions

![](Challenge7_files/figure-gfm/unnamed-chunk-5-1.png)<!-- -->

    According to this plot, letters denotes significant differences using the p-value cut off of 0.05. Treatment 3 and 5 are signifcantly different from treatment 7 and 2. HOWEVER, no difference is treatments 1,3,4,5,6,8 and 9.
